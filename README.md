...
<img src="assets/logo.svg" alt="Void" width="120">
...
<p align="center">
  <img src="docs/images/hero-dark.png" alt="Void main window">
</p>
...
- **macOS-native** — Tauri v2 with hardened runtime, universal binary (~50 MB DMG), menu-bar resident.

## Install

Void targets macOS 12+ (Apple Silicon and Intel, single universal binary).

1. Download the latest `Void_X.Y.Z_universal.dmg` from [Releases](https://github.com/intergalactisch/void/releases/latest).
...
<!-- Screenshots land here once Phase I (designer) captures them. Filenames per docs/SCREENSHOTS.md. -->

| | |
|---|---|
| ![Capture](docs/images/capture-dark.png) | **Capture** — Cmd+Shift+Enter opens a quick-capture window that drops a timestamped block into your inbox or daily note. |
| ![Distill](docs/images/distill-dark.png) | **Distill** — `/distill` rewrites the current note into a tighter version, recording the transformation in `.void/provenance/`. |
| ![Branch](docs/images/branch-dark.png) | **Branch** — explore alternative versions of a note side by side without losing the original. Merge or drop with one click. |
| ![Bridge](docs/images/bridge-dark.png) | **Bridge** — `/bridge` synthesises a paragraph that connects two notes, with sources you can audit. |
| ![Thread](docs/images/thread-dark.png) | **Thread** — `/thread <topic>` orders notes that touch the topic into a coherent reading sequence with AI commentary. |
