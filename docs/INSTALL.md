...
Void is a Tauri v2 desktop app. It ships only for macOS today (Apple Silicon and Intel, separate ~25 MB DMGs per architecture).

## 1. Download

Go to [Releases](https://github.com/intergalactisch/void/releases/latest) and grab the DMG that matches your Mac:

- **Apple Silicon** (M1 / M2 / M3 / M4) → `Void_X.Y.Z_aarch64.dmg`
- **Intel** → `Void_X.Y.Z_x64.dmg`

Not sure which you have? Open the Apple menu → **About This Mac**. The "Chip" line will read either *"Apple M…"* (Apple Silicon) or *"Intel Core…"*.
